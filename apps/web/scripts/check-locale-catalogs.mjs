...
const LOCALES = ["zh-cn.ts", "en.ts", "zh-tw.ts", "fr.ts", "ru.ts", "ja.ts", "vi.ts"];
const INDEX_EXPORTS = ["zhCN", "en", "zhTW", "fr", "ru", "ja", "vi"];
...
function checkContentLocalesDir(relDir) {
  ...
  ok(`${relDir}/content-locales (7 files + index)`);
}
...
for (const rel of uiCopyFiles) {
  ...
  for (const loc of ['"zh-CN"', "en,", '"zh-TW"', "fr,", "ru,", "ja,", "vi,"]) {
...
for (const field of ["consoleOnlyHint"]) {
  if (!authContent.includes(field)) {
    fail(`auth/login/content.ts missing auth field ${field}`);
  }
}
ok("auth/login/content.ts Target Locale set (14)");

const languagesPath = path.join(root, "src/lib/languages.ts");
const languages = fs.readFileSync(languagesPath, "utf8");
for (const marker of ["ko:", "de:", "es:", '"pt-BR"', "ar:", "hi:", "id:", "consoleOnly:"]) {
  if (!languages.includes(marker)) {
    fail(`languages.ts may missing Target locale ${marker}`);
  }
}
ok("languages.ts Target Locale entries");

const localeUiPath = path.join(root, "src/lib/locale-ui-copy.ts");
if (!fs.existsSync(localeUiPath)) {
  fail("missing src/lib/locale-ui-copy.ts");
} else {
  const localeUi = fs.readFileSync(localeUiPath, "utf8");
  for (const marker of TARGET_LOCALE_MARKERS) {
    if (!localeUi.includes(marker)) {
      fail(`locale-ui-copy.ts missing ${marker}`);
    }
  }
  ok("locale-ui-copy.ts (14 locales)");
}

const localeRegistry = fs.readFileSync(path.join(root, "src/lib/locale.ts"), "utf8");
if (!localeRegistry.includes("TARGET_LOCALES") || !localeRegistry.includes("CONSOLE_LOCALES")) {
  fail("locale.ts missing TARGET_LOCALES or CONSOLE_LOCALES");
} else {
  ok("locale.ts Target / Console Locale registry");
}
